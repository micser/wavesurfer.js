...
class Renderer extends EventEmitter<RendererEvents> {
  ...
  async renderDummyWaveform() {
    ...
    for (let channel = 0; channel < dummyAudio.numberOfChannels; channel++) {
      const nowBuffering = dummyAudio.getChannelData(channel);
      for (let i = 0; i < dummyAudio.length; i++) {
        nowBuffering[i] = Math.random() * 2 -1;
      }
      dummyAudio.copyFromChannel(nowBuffering, channel)
    }
    this.render(dummyAudio)
  }
